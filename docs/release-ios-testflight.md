...
down so it can be audited before an upload, current release build 60 (1.0.3,
3D avatars).
...
xcodebuild -project OpenClamLiveKit.xcodeproj -scheme OpenClamLiveKit \
  ...
  -archivePath build/OpenClam-1.0.3-60.xcarchive archive
xcodebuild -exportArchive -archivePath build/OpenClam-1.0.3-60.xcarchive \
  -exportOptionsPlist ExportOptions-AppStore.plist -exportPath build/export-60
```
...
- Note the build in the release commit (`release(ios): ship build 60`).
