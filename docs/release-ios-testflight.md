...
down so it can be audited before an upload, current release build 78 (1.0.6,
...
xcodebuild -project OpenClamLiveKit.xcodeproj -scheme OpenClamLiveKit \
  ...
  -archivePath build/OpenClam-1.0.6-78.xcarchive archive
xcodebuild -exportArchive -archivePath build/OpenClam-1.0.6-78.xcarchive \
  -exportOptionsPlist ExportOptions-AppStore.plist -exportPath build/export-78
```
...
- Note the build in the release commit (`release(ios): ship build 78`).
