...
import { applyOpenClamAccountConfig, getOpenClamConfig, listOpenClamAccountIds, openClamPluginConfigSchema, resolveDefaultOpenClamAccountId, resolveOpenClamAccount, } from "./config.js";
export function createOpenClamChannelBase() {
    return {
        id: "openclam",
        meta: {
            id: "openclam",
        ...
        config: {
            ...
            hasConfiguredState: ({ cfg }) => listOpenClamAccountIds(cfg).some((accountId) => resolveOpenClamAccount(cfg, accountId).configured),
        },
        setup: {
            resolveAccountId: ({ cfg, accountId }) => accountId ?? resolveDefaultOpenClamAccountId(cfg),
            resolveBindingAccountId: ({ accountId }) => accountId,
            applyAccountConfig: ({ cfg, accountId, input }) => applyOpenClamAccountConfig({ cfg, accountId, input: input }),
            validateInput: ({ accountId, input }) => {
                const agentId = typeof input.userId === "string" ? input.userId.trim() : "";
                if (!accountId.trim())
                    return "An OpenClam account ID is required.";
                if (!agentId)
                    return "Choose an OpenClaw agent, or use `openclaw openclam pair`.";
                return null;
            },
        },
        status: {
...
export function configuredOpenClamAccounts(cfg) {
    return listOpenClamAccountIds(cfg).map((accountId) => resolveOpenClamAccount(cfg, accountId));
}
