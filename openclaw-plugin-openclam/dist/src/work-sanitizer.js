...
export function sanitizeWorkText(value, maximum) {
...
export function sanitizeWorkPath(value) {
    const candidate = compact(value).replaceAll("\\", "/");
    if (!candidate || candidate.length > 512 || !RELATIVE_PATH.test(candidate))
        return undefined;
    return truncateUnicode(candidate, 512);
}
export function workStepId(...parts) {
...
export function sanitizeWorkStep(step) {
...
export function workState(value, phase) {
    const normalized = `${compact(value)} ${compact(phase)}`.toLowerCase();
    if (/(fail|error|denied|reject)/u.test(normalized))
        return "failed";
    if (/(wait|approval|pending)/u.test(normalized))
        return "waiting";
    if (/(complete|completed|done|success|finished|end)/u.test(normalized))
        return "completed";
    return "running";
}
