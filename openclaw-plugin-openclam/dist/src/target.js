...
export function parseOpenClamTarget(raw) {
    ...
    if (!connectionId || !conversationId)
        return undefined;
    if (!UUID_PATTERN.test(connectionId) || !UUID_PATTERN.test(conversationId))
        return undefined;
    return {
        connectionId,
        conversationId,
...
export const openClamMessaging = {
    ...
    targetResolver: {
        looksLikeId: (raw, normalized) => Boolean(parseOpenClamTarget(normalized ?? raw)),
