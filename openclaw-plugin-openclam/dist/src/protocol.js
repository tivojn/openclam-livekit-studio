...
import { IDENTIFIER_PATTERN, UUID_PATTERN, } from "./types.js";
...
export function parseBridgeInbound(raw, expectedConnectionId) {
    ...
    if (isRecord(value) && value.kind === "relay.persisted") {
        if (!exactKeys(value, ["v", "kind", "connectionId", "payload"]) ||
            value.v !== 1 ||
    ...
    if (!isRecord(value) ||
        !exactKeys(value, ["v", "kind", "connectionId", "messageId", "seq", "sentAt", "payload"], ["conversationId", "replyTo"]) ||
        value.v !== 1 ||
        typeof value.kind !== "string" ||
        !FRAME_KINDS.has(value.kind) ||
        value.connectionId !== expectedConnectionId ||
        !UUID_PATTERN.test(String(value.connectionId)) ||
    ...
    if (kind === "heartbeat") {
        if (!exactKeys(value.payload, [], ["lastReceivedSeq"]))
            throw new Error("invalid_frame");
        if (value.payload.lastReceivedSeq !== undefined &&
            !validSafeInteger(value.payload.lastReceivedSeq, 0)) {
            throw new Error("invalid_frame");
        }
        return value;
    }
    if (!UUID_PATTERN.test(String(value.conversationId)))
        throw new Error("invalid_frame");
    if (!exactKeys(value.payload, kind === "turn.submit" ? ["turnId", "accountId", "text"] : ["turnId"], kind === "turn.submit" ? ["capabilities"] : [])) {
        throw new Error("invalid_frame");
    }
    if (!UUID_PATTERN.test(String(value.payload.turnId)))
        throw new Error("invalid_frame");
    if (kind === "turn.submit") {
        if (!IDENTIFIER_PATTERN.test(String(value.payload.accountId)) ||
...
export function parseClientFrame(raw, expectedConnectionId) {
    const value = parseBridgeInbound(raw, expectedConnectionId);
    if (value.kind === "relay.persisted")
        throw new Error("invalid_client_kind");
    return value;
}
export function parseCreatePairingResponse(value) {
    if (!isRecord(value) ||
...
export function encodeFrameWithTextBudget(frame, payloadKey) {
    const text = frame.payload[payloadKey];
    if (typeof text !== "string")
        return { frame, encoded: encodeFrame(frame) };
