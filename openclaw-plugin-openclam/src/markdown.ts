...
function markdownLinkEnd(text: string, destinationStart: number): number | undefined {
  let depth = 1;
  ...
  for (let cursor = destinationStart; cursor < text.length; cursor += 1) {
    ...
    if (!sawDestinationCharacter && /\s/u.test(character)) continue;
...
export function rewriteMarkdownLinks(
  text: string,
  rewrite: (link: MarkdownLink) => string | undefined,
): string {
  let output = "";
  let copiedThrough = 0;
