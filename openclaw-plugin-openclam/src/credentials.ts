...
import { type AdapterState, UUID_PATTERN } from "./types.js";

const TOKEN_PATTERN = /^[A-Za-z0-9_-]{40,128}$/u;

export function defaultCredentialPaths(connectionId: string): {
...
export async function writeAdapterCredential(path: string, token: string): Promise<void> {
  if (!TOKEN_PATTERN.test(token)) throw new Error("invalid_adapter_token");
  await writePrivateFile(path, `${token}\n`);
}

export async function readAdapterCredential(path: string): Promise<string> {
  const info = await stat(path);
  if (!info.isFile()) throw new Error("adapter_token_not_file");
...
export function initialAdapterState(connectionId: string): AdapterState {
  return {
    v: 1,
    connectionId,
    nextSeq: 1,
...
function parseAdapterState(value: unknown, connectionId: string): AdapterState {
  if (typeof value !== "object" || value === null || Array.isArray(value)) {
    throw new Error("invalid_adapter_state");
  }
  const candidate = value as Partial<AdapterState>;
  if (
    candidate.v !== 1 ||
  ...
  return candidate as AdapterState;
}

export async function readAdapterState(path: string, connectionId: string): Promise<AdapterState> {
  const info = await stat(path);
  if (!info.isFile()) throw new Error("adapter_state_not_file");
  if (process.platform !== "win32" && (info.mode & 0o077) !== 0) {
...
export async function writeAdapterState(path: string, state: AdapterState): Promise<void> {
