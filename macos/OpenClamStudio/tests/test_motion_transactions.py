...
class WalkCycleContractTests(unittest.TestCase):
    ...
    def test_production_walk_prompts_keep_both_arms_trackable(self):
        ...
        self.assertIn("LEFT leg and foot slightly forward", keyframe)
        self.assertIn("RIGHT arm and hand slightly forward", keyframe)
        self.assertIn("RIGHT leg and foot slightly behind", keyframe)
        self.assertIn("LEFT arm and hand slightly behind", keyframe)
        self.assertIn("full alternating contralateral cycle", video)
        for prompt in (keyframe, video):
            self.assertIn("left leg", prompt.lower())
            self.assertIn("right arm", prompt.lower())
            self.assertIn("right leg", prompt.lower())
            self.assertIn("left arm", prompt.lower())
            self.assertIn("ipsilateral", prompt.lower())
            self.assertIn("same-side", prompt.lower())
            self.assertIn("顺拐", prompt)
