...
(async()=>{
 ...
 lib.avatar.motion.clips=new Map(['walk','casual-walk','stage-walk','wave'].map(id=>[id,{}]));
 lib.select({body:'pose',prop:'prop',walkStyle:'stage-walk'},75000);lib.update(76000);
 assert.equal(lib.walkingClip(),'stage-walk');
 const selectedPose=lib.current,transition=lib.transition;
 lib.select({...lib.selection,walkStyle:'casual-walk'},76100);
 assert.equal(lib.walkingClip(),'casual-walk');assert.equal(lib.current,selectedPose);assert.equal(lib.transition,transition,'changing travel style does not reset the standing pose');
 assert.equal(lib.selection.prop,'prop');assert(lib.isTravelClip('stage-walk')&&!lib.isTravelClip('wave'));
 lib.select({...lib.selection,walkStyle:'wave'},76200);assert.equal(lib.walkingClip(),'walk','only locomotion styles are accepted');
 lib.avatar.motion.clips.set('walking-woman',{});
 assert.equal(lib.walkingClip(),'walking-woman','Walking Woman is the default whenever this asset is installed');
 assert.equal(lib.catalogue().walkingStyles[0].id,'walking-woman');
 assert(lib.isTravelClip('walking-woman'),'the default gait must keep travelling instead of blocking its own next frame');
 lib.select({...lib.selection,walkStyle:'walking-woman'},76300);assert.equal(lib.walkingClip(),'walking-woman');
 lib.select({...lib.selection,walkStyle:'stage-walk'},76400);assert.equal(lib.walkingClip(),'stage-walk','a manual choice still overrides the default');
 lib.select({...lib.selection,walkStyle:''},76500);assert.equal(lib.walkingClip(),'walking-woman','resetting the picker returns to Walking Woman');
 lib.avatar.motion.clips.delete('walking-woman');assert.equal(lib.walkingClip(),'walk','older packages keep their supported walk');
