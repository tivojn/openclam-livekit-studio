...
scrub_generated_bytecode() {
  find "$OUT" -type d -name __pycache__ -prune -exec /bin/rm -rf {} +
  find "$OUT" -type f \( -name '*.pyc' -o -name '*.pyo' -o -name '.DS_Store' \) -delete
}
...
require_clean_runtime() {
  ...
  generated="$(find "$OUT" \
    ...
    -print -quit)"
  if [[ -n "$generated" ]]; then
    echo "generated Python bytecode remains in staged runtime: $generated" >&2
    exit 1
  fi
}

require_no_private_home() {
  local private_home scan_status
  private_home="$(python3 -c 'from pathlib import Path; print(Path.home())')"
  if /usr/bin/grep -R -a -l -F -- "$private_home" "$OUT" >/dev/null; then
    echo "private home path remains in staged Python runtime" >&2
    exit 1
  else
    scan_status=$?
    if [[ $scan_status -ne 1 ]]; then
      echo "could not complete the staged Python private-path scan" >&2
      exit 1
    fi
  fi
}

smoke_runtime_without_bytecode() {
  PYTHONDONTWRITEBYTECODE=1 "$OUT/bin/python" -B -c \
    'import encodings, json, site, ssl, sys; assert sys.version_info[:2] == (3, 12)'
  require_clean_runtime
  require_no_private_home
}

if [[ "${1:-}" == "--scrub-only" ]]; then
  [[ "$#" -eq 1 ]] || { echo 'usage: stage-electron-python.sh [--scrub-only]' >&2; exit 2; }
  [[ -x "$OUT/bin/python" ]] \
    || { echo "staged Python runtime is missing: $OUT" >&2; exit 1; }
  scrub_generated_bytecode
  require_clean_runtime
  smoke_runtime_without_bytecode
  echo "scrubbed staged portable Python runtime"
  exit 0
fi
[[ "$#" -eq 0 ]] || { echo 'usage: stage-electron-python.sh [--scrub-only]' >&2; exit 2; }
...
replacement = b"/opt/openclam-python"
remaining = []
