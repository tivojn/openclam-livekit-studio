...
DENIED_CODEC_FILENAMES = re.compile(
    r"^(?:libav(?:codec|format)|libx26[45]|librubberband|libvidstab)",
    re.IGNORECASE,
)
...
def main() -> int:
    ...
    for path in root.rglob("*"):
        ...
        if path.is_file() and DENIED_CODEC_FILENAMES.search(path.name):
            failures.append(f"denied bundled GPL-capable codec library: {path.relative_to(root)}")
