...
enum OpenClamAvatarStoreReleasePolicy {
    static let productionCatalogURL = URL(
        string: "https://raw.githubusercontent.com/tivojn/openclam-livekit-studio/avatar-store-v1.0.1/shared/avatar-store-v1/catalog/v1/catalog.json"
    )!
...
enum OpenClamAvatarStoreURLPolicy {
    ...
    static func allowsCatalogURL(_ url: URL) -> Bool {
        guard hasSafeHTTPSComponents(url),
              ...
              url.path == "/\(owner)/\(repository)/avatar-store-v1.0.1/shared/avatar-store-v1/catalog/v1/catalog.json" else {
            return false
        }
        return true
    }
